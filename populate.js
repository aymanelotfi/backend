...
async function fetchMoviesFromTheMovieDatabase() {
  ...
  for (var i = 1; i <= 50; i++) {
	  try{
    	movielist.push(
      await axios.get(
        "https://a...content-available-to-author-only...b.org/3/movie/popular?api_key=522d421671cf75c2cba341597d86403a&language=en-US&page=" + i));
		} catch(err) {
			console.log(err);
		}
  }
  ...
  for (var i = 0; i < 50; i++) {
    for (var j = 0; j < 20; j++) {
      if (
        ...
        movies[i].data.results[j].release_date != null && movies[i].data.results[j].title != "Sonic the Hedgehog 3"
      ) {
